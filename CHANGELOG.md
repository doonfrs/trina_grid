...
* Perf: Filtering and number formatting allocate far less per cell. The row filter predicate resolves each condition's column, type and search text once instead of scanning the enabled columns for every cell of every row; `Contains`, `Equals`, `StartsWith`, `EndsWith` and `Regex` reuse their compiled `RegExp` instead of compiling one per cell; number-family columns format a `num` without converting it to a string and parsing it back, and reuse the pattern that strips a formatted value. On 150,000 cells (host JIT) an all-columns `Contains` predicate over 18 text columns went from 470 ms to 348 ms, currency cell initialization from 164 ms to 142 ms and `applyFormat` of a `num` from 81 ms to 45 ms. Results are unchanged. @jan-siroky
