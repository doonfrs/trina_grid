...
* Feature: The column filter row now resolves its widget from the column type. Boolean columns render an ALL / True / False dropdown filter and select columns (with `enableColumnFilter: true`, the new default of the previously unused flag) render a checkbox multi-select dropdown that re-filters live on every toggle and includes a Select all toggle. Any other column keeps the text filter. Override with the new `TrinaFilterColumnWidgetDelegate.booleanSelect()` / `.multiSelect(...)` constructors, or opt out with `.textField()`. Dropdown labels are localizable via `TrinaGridLocaleText.filterAll` / `filterTrue` / `filterFalse` / `filterSelectAll`. @doonfrs
* Fix: `TrinaGridChangeColumnFilterEvent` now also updates the filter type of an existing filter row for the column, not just its value. A row previously created with another type (e.g. Contains through the filter popup or `setColumnFilter`) kept comparing with the old semantics when the filter row widget sent an update. @doonfrs
